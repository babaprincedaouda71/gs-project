...
export class InvoiceComponent implements OnInit, OnDestroy {
  ...
  generateInvoice(invoice: InvoiceModel) {
    ...
    if (invoice.invoiceType === 'groupInvoice') {
      this.invoicingService.generateGroupsInvoice(invoice.numberInvoice, invoice, invoice.groups, invoice.client);
    }
  }
...
export class InvoiceDialogContentComponent {
  ...
  doAction() {
    if (this.action == 'status') {
      ...
      this.groupService.updateGroupsStatus(this.local_data.groupsIds)
        .subscribe({
          next: value => {
