...
export class EditTrainingComponent implements OnInit, OnDestroy {
  ...
  private buildEditForm() {
    this.updateTrainingForm = this.formBuilder.group({
      ...
      totalNumberOfDays: [this.training.totalNumberOfDays],
      totalStaff: [this.training.totalStaff],
  ...
  handleDateToggle(groupIndex: number, isChecked: boolean) {
    const group = (this.updateTrainingForm.get('groups') as FormArray).at(groupIndex);

    if (isChecked) {
      // Si on passe aux dates simples, on vide l'intervalle de dates
      group.patchValue({
        startDate: null,
        endDate: null
      });

      // S'assurer qu'il y a au moins une date dans le tableau
      if ((group.get('groupDates') as FormArray).length === 0) {
        this.addDate(groupIndex);
      }
    } else {
      // Si on passe à l'intervalle de dates, on vide le tableau de dates
      const datesArray = group.get('groupDates') as FormArray;
  ...
  onSubmit() {
    ...
    training.amount = this.updateTrainingForm.get('dailyAmount')?.value * this.updateTrainingForm.get('totalNumberOfDays')?.value
    this.groupService.calculateGroupAmount(training)
