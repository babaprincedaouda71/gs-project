...
export class AddTrainingComponent implements OnInit, OnDestroy {
  ...
  * */
  buildTrainingForm() {
    this.addTrainingForm = this.formBuilder.group({
      ...
      totalNumberOfDays: ['', [Validators.required, positiveNumberValidator]],
  ...
  * */
  onSubmit() {
    ...
    training.amount = this.addTrainingForm.get('dailyAmount')?.value * this.addTrainingForm.get('totalNumberOfDays')?.value
